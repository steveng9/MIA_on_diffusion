...
def attack_VAE():
    ...
    ## Train VAE
    tabsynS.instantiate_vae(
        **raw_config["model_params"], optim_params=raw_config["train"]["optim"]["vae"]
    )
    tabsynA.instantiate_vae(
        **raw_config["model_params"], optim_params=raw_config["train"]["optim"]["vae"]
    )

    os.makedirs(f"{MODEL_PATH_S}/{DATA_NAME}/vae", exist_ok=True)
    os.makedirs(f"{MODEL_PATH_A}/{DATA_NAME}/vae", exist_ok=True)
    ...
    tabsynS.train_vae(
        **raw_config["loss_params"],
        ...
        # num_epochs=200,
        num_epochs=1,
        save_path=os.path.join(MODEL_PATH_S, DATA_NAME, "vae"),
    )
    tabsynA.train_vae(
        **raw_config["loss_params"],
        # num_epochs=raw_config["train"]["vae"]["num_epochs"],
        num_epochs=200,
        # num_epochs=1,
        save_path=os.path.join(MODEL_PATH_A, DATA_NAME, "vae"),
    )

    # embed all inputs in the latent space
    tabsynS.save_vae_embeddings(
        XS_train_num, XS_train_cat, vae_ckpt_dir=os.path.join(MODEL_PATH_S, DATA_NAME, "vae")
    )
    tabsynA.save_vae_embeddings(
        XA_train_num, XA_train_cat, vae_ckpt_dir=os.path.join(MODEL_PATH_A, DATA_NAME, "vae")
    )
    # TODO: save embeddings for challenge in Synthetic TabSyn model and Aux TabSyn model.!

    challenge_mse_lossS, challenge_ce_lossS, challenge_kl_lossS, challenge_accS = tabsynS.attack_vae(XC_test_num, XC_test_cat)
    challenge_mse_lossA, challenge_ce_lossA, challenge_kl_lossA, challenge_accA = tabsynA.attack_vae(XC_test_num, XC_test_cat)

    # with open(LOSS_RESULTS + f'synth_losses{model_num}.pkl', 'wb') as file:
    #     pickle.dump((challenge_mse_lossS, challenge_ce_lossS, challenge_kl_lossS, challenge_accS), file)
    # with open(LOSS_RESULTS + f'aux_losses{model_num}.pkl', 'wb') as file:
    #     pickle.dump((challenge_mse_lossA, challenge_ce_lossA, challenge_kl_lossA, challenge_accA), file)
